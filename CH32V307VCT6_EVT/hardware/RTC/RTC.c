...
void RTC_Init(void)
{
    ...
    if(BKP_ReadBackupRegister(BKP_DR1)!=0XA1A3)
    {
            RCC_Configuration();
            RTC_Set(2023,7,30,13,16,30);
            //GPIO_SetBits(GPIOD, GPIO_Pin_13);//点亮D1
            BKP_WriteBackupRegister(BKP_DR1, 0XA1A3);//向执行的后备寄存器中写入用户程序数据
    }
    else
    {
        ...
        //RTC_SetAlarm(60+RTC_GetCounter());
